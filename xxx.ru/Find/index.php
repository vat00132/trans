...
$rows = count($obj);
if(isset($_POST['list1'])) {$list1=$_POST['list1']; unset($list1); }
$list1=$_POST['list1'];
if(isset($_POST['find_text'])) {$find_text=$_POST['find_text']; unset($find_text); }
...
<div class="transaction" align="center">  
            ...
            <?php
                for( $i = 0; $i < $rows; $i++)
                {
                    ...
                    {
                        case "null": $flag=0;
                        case "transaction_project_name": if($transaction_project_name==$find_text) $flag=1; break;
                        case "payment_method_name": if($payment_method_name==$find_text) $flag=1; break;
                        case "refund_reason": if($refund_reason==$find_text) $flag=1; break;
                        case "status": if($status==$find_text) $flag=1; break;
                        case "user_name": if($user_name==$find_text) $flag=1; break;
                        case "user_email": if($user_email==$find_text) $flag=1; break;
                        case "user_country": if($user_country==$find_text) $flag=1; break; 
                        case "payment_amount": if($payment_amount==$find_text) $flag=1; break;
                        case "the_amount_of_PS": if($the_amount_of_PS==$find_text) $flag=1; break;
                        case "purchase_virtual_currency_name": if($purchase_virtual_currency_name==$find_text) $flag=1; break;
                        case "purchase_virtual_items": if($purchase_virtual_items==$find_text) $flag=1; break;
                        case "purchase_simple_checkout_amount": if($purchase_simple_checkout_amount==$find_text) $flag=1; break;
                        case "purchase_simple_checkout_currency": if($purchase_simple_checkout_currency==$find_text) $flag=1; break;
                        case "purchase_subscription_name": if($purchase_subscription_name==$find_text) $flag=1; break;
                    }
                    if($flag==1)
                    {
                    ?>
